		...
		for i in range(4):
			for j in range(4):
				...
				for k in range(4):
					...
					b = state_matrix[k][j]
					rez = b
					print(f"{i},{k}*{k},{j}\thex\tdec\tbin\t")
					print(f" a:\t{a}\t{(a)}\t{np.binary_repr(int(a))}")
					print(f" b:\t{hex(int_from_hex(b))[2:]}\t{(int_from_hex(b))}\t{np.binary_repr(int_from_hex(b))}\t")
					if a == '1':
						rez = int_from_hex(b)
						print(f"=>({k}): 1*b = 1 * {np.binary_repr(int_from_hex(b))} = {np.binary_repr(rez)}")
					elif a == '2':
						b = (int_from_hex(b))
						rez = mul_by_02(b)
...
def key_expansion_schedule(key):
	...
	print("\nKey Expansion:\n--------------------------------------------------------------------")
	print("• First 4 words are first sublets of 8 chars in key")
	...
	for i in range(4, 44):
		...
		if i % 4 == 0:
			...
			print(f" 3) rcon_{int(i/4)} * w")
			print(f" {''.join(y)} ⊕ {hex(rcon)[:4]} = {''.join(temp)}")
			print(f" g(w[{temp_index}])] = ", ''.join(temp))  
			print(f" w[{temp_index+1}] =  w[{word_index}] ⊕ g(w[{temp_index}])")
		print()
...
def add_rounkeys(roundkeys, state_matrix, inv):
	...
	rk_index = 0
	if inv is True: rk_index = 10
	print(f" 1) ⊕ RK_matrix[{rk_index}]:")
...
def main():
	...
	# my
	#key		  = ["54", "45", "4C", "45", "56", "49", "5A", "4F", "52", "55", "4C", "20", "45", "20", "48", "44"]
	#plaintext = ["44", "4F", "52", "49", "4E", "41", "20", "41", "52", "45", "20", "43", "41", "49", "53", "45"]
